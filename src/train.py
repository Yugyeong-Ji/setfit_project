...
for seed in range(num_seeds):
    ...
    for i in range(num_iterations):
        for ex in few_shot:
            ...
            label = ex["label"]
            # positive -> same label
            pos = random.choice([e for e in few_shot if e["label"] == label and e != ex])
            # negative -> different label
            neg = random.choice([e for e in few_shot if e["label"] != label])
            train_examples.append(InputExample(texts=[anchor, pos["sentence"]], label=1.0))
            train_examples.append(InputExample(texts=[anchor, neg["sentence"]], label=0.0))
